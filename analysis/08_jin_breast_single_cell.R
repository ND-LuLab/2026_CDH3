...
config <- list(
  output_root = "results",
  inputs = list(
    jin_all = "data/single_cell/Jin.rds"
  )
)
...
# Topic: Jin single-cell analysis and visualization. Fig2E-F.
...
library(grid)
dataset <- "Jin"
out.dir <- paste0(output_dir("08_jin_breast_single_cell"), "/")
ds <- readRDS(input_file("jin_all"))
...
ds@meta.data$celltype[which(ds@meta.data$seurat_clusters %in% c(10,25))] <- 'Basal epithelial cells'
ds@meta.data$celltype[which(ds@meta.data$seurat_clusters %in% c(2,16,17))] <- 'B cells'
ds@meta.data$celltype[which(ds@meta.data$seurat_clusters == 3)] <- 'NK cells'
ds@meta.data$celltype[which(ds@meta.data$seurat_clusters == 21)] <- 'pDCs'
ds@meta.data$celltype[which(ds@meta.data$seurat_clusters == 22)] <- 'fDCs'
ds@meta.data$celltype[which(ds@meta.data$seurat_clusters %in% c(19,24))] <- 'cDCs'
ds@meta.data$celltype[which(ds@meta.data$seurat_clusters %in% c(0,7,5,1,8,15))] <- 'T cells'
ds@meta.data$celltype[which(ds@meta.data$seurat_clusters == 6)] <- 'Neutrophils'
ds@meta.data$celltype[which(ds@meta.data$seurat_clusters == 12)] <- 'Macrophages'
ds@meta.data$celltype[which(ds@meta.data$seurat_clusters == 13)] <- 'Monocytes'
ds@meta.data$celltype[which(ds@meta.data$seurat_clusters == 9)] <- 'Fibroblasts'
Idents(ds) <- ds@meta.data$seurat_clusters
ds <- subset(ds, idents = setdiff(levels(Idents(ds)), c(11,18))) # remove mitochodrial population
...
ds@meta.data$celltype <- factor(ds@meta.data$celltype, levels = c("Basal epithelial cells", "Luminal epithelial cells",
                                                                 "T cells","NK cells","B cells","Macrophages","Monocytes","Neutrophils","pDCs","cDCs","fDCs",
                                                                 "Fibroblasts","Endothelial cells","Platelets","Proliferating cells"))
my_colors <- p_colors
my_colors[11] <- p_colors[1]
...
ds.epi <- subset(ds, subset = seurat_clusters %in% c(10) & UMAP_1 < 10 & UMAP_1 > 4 & UMAP_2 > 4 & UMAP_2 < 10)

celltype <- rep('Luminal',nrow(ds.epi@meta.data))
celltype[which(ds.epi$seurat_clusters %in% c(10))] <- "Basal"
...
ds@meta.data$compare <- rep('Others',nrow(ds@meta.data))
ds@meta.data$compare[which(ds@meta.data$celltype == 'Basal epithelial cells')] <- "Basal"
dp <- DotPlot(ds,features = c("CDH3"), group.by = "compare") +
...
pdf(paste0(out.dir,dataset,"_CDH3_dot_compare_others.pdf"), width = 3, height = 4)
print(dp)
dev.off()
...
print(p)
dev.off()
jpeg(filename = paste0(out.dir,dataset,"_",gene,".jpg"), width = 4, height = 4, units = 'in', res = 300)
print(p)
dev.off()
